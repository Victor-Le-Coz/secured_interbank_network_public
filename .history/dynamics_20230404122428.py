...
class ClassDynamics:
    def __init__(
        ...
        df_banks_data = pd.DataFrame(index=range(self.Network.nb_banks))
    ...
    def fill_df_network_trajectory(self):
        ...
        for i, Bank in enumerate(self.Network.banks):
            ...
            for key in Bank.off_bs_items.keys():  # only loop over off-balance
                # items.
                self.df_network_trajectory.loc[
                    self.Network.step, key + " tot. volume"
                ] += Bank.off_bs_items[key]
                self.dic_banks_off_balance[key][i] = Bank.off_bs_items[key]
