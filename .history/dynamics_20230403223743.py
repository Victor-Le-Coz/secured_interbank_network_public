...
class ClassDynamics:
    def __init__(
        ...
        self.df_bank_trajectory = pd.DataFrame(index=self.nb_steps,columns=par.bank_metrics)
        ...
        self.df_network_trajectory = pd.DataFrame(index=self.nb_steps, columns=par.network_metrics)
    ...
    def comp_single_trajectory(self):
        ...
        for key in self.df_bank_trajectory.keys():
            self.df_bank_trajectory[key].append(0.0)

        # Build the time series of the accounting item of the bank bank_id
        for key in bank.assets.keys():
            self.df_bank_trajectory[key][-1] = bank.assets[key]
        for key in bank.liabilities.keys():
            self.df_bank_trajectory[key][-1] = bank.liabilities[key]
        for key in bank.off_balance.keys():
            self.df_bank_trajectory[key][-1] = bank.off_balance[key]
        ...
        self.df_bank_trajectory["Av. in-degree"][-1] = bank_network.in_degree(
            self.single_bank_id
        )
        self.df_bank_trajectory["Av. out-degree"][
            -1
        ] = bank_network.out_degree(self.single_bank_id)

        # Number of transactions of end repos per step
        self.df_bank_trajectory["Nb. of repo transactions ended"][
            -1
        ...
        if (
            ...
            self.df_bank_trajectory["Av. volume of repo transactions ended"][
                -1
            ] = (
        ...
        self.df_bank_trajectory["Repos av. maturity"][-1] = np.sum(
            list(
                np.array(
    ...
    def save_step_figures(self):
        ...
        gx.plot_collateral(self.df_network_trajectory, self.result_location)
        ...
        gx.plot_excess_liquidity_and_deposits(
            self.df_network_trajectory, self.result_location
        )
        ...
        gx.plot_gini(self.df_network_trajectory, self.result_location)
        ...
        gx.plot_degre_network(self.df_network_trajectory, self.result_location)
        ...
        gx.plot_single_trajectory(
            self.df_bank_trajectory, self.result_location
        )

    def simulate(self,save_every=10, output_keys=False):
        """
        ...
        for _ in tqdm(range(self.nb_steps)):
