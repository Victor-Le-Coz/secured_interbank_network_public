    ...
    def fill(self):
        for i, Bank in enumerate(self.banks):
            ...
            self.dic_matrices["adjency"][
                self.dic_matrices["adjency"] < 0
            ] = 0  # take out -e-14 negative values
            self.dic_matrices["non-zero_adjency"] = self.dic_matrices[
                "adjency"
            ][
                np.nonzero(self.dic_matrices["adjency"])
            ]  # keep only non zero reverse repos
            self.dic_matrices["trust"][i, :] = np.array(
                list(self.banks[i].trust.values())
            )
            self.dic_matrices["trust"][i, i] = 0
            self.dic_matrices["binary_adjency"][i, :] = np.where(
