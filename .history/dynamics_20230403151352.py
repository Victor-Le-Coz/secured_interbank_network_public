...
class ClassDynamics:
    ...
    def simulate(self, time_steps, save_every=10, output_keys=None):
        ...
        :return:
        """
        self.Network.save_param(time_steps, save_every)
        for _ in tqdm(range(time_steps)):
            if self.Network.step % save_every == 0.0:
                # Update all the metrics at time step 0
                self.Network.comp_step_metrics()
                self.Network.comp_single_trajectory()
                self.Network.save_step_figures()
        ...
        #     )
        self.Network.save_step_figures()
        self.Network.comp_final_metrics()

        output = self.Network.build_output(output_keys)
        return output
