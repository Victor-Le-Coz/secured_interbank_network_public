...
class ClassDynamics:
    def __init__(
        ...
        self.self.dic_matrices["adjency"] = np.zeros(
    ...
    def fill_df_network_trajectory(self):
        ...
        for i, Bank in enumerate(self.Network.banks):
            ...
            self.self.dic_matrices["adjency"][i, :] = np.array(
    ...
    def record_trajectories(self):
        ...
        self.self.dic_matrices["adjency"][
            self.self.dic_matrices["adjency"] < 0
        ] = 0

        # build a binary adjency matrix from the weighted adjency matrix
        binary_adj = np.where(
            self.self.dic_matrices["adjency"] > self.Network.min_repo_size,
            True,
            False,
        )
        ...
        non_zero_adj_matrix = self.self.dic_matrices["adjency"][
            np.nonzero(self.self.dic_matrices["adjency"])
    ...
    def store_trajectories(self):
        ...
        self.Network.dic_matrices["binary_adjency"] = np.where(
            self.self.dic_matrices["adjency"] > self.Network.min_repo_size,
            1.0,
            0.0,
        )
        gx.plot_network(
            adj=self.self.dic_matrices["adjency"],
