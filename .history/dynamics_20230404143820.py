...
class ClassDynamics:
    ...
    def fill_df_network_trajectory(self):
        ...
        for i, Bank in enumerate(self.Network.banks):
            ...
            self.Network.dic_matrices["adjency"][i, :] = np.array(
            ...
            self.Network.dic_matrices["trust"][i, :i] = trusts[:i]
            self.Network.dic_matrices["trust"][i, i + 1 :] = trusts[i:]

            # Build the total assets of each bank
            self.Network.df_banks["Total assets"][i] = self.Network.banks[
                i
            ].total_assets()

            # Build the deposits numpy array of each bank
            self.Network.df_banks["Deposits"][i] = self.Network.banks[
                i
    ...
    def record_trajectories(self):
        ...
        binary_adj = np.where(
            self.Network.dic_matrices["adjency"] > self.Network.min_repo_size,
            True,
            False,
        )
