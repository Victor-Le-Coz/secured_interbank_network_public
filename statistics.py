...
# Definition of the class ClassHistory
class ClassHistory:
    """
    Object allowing the recording of the whole information of the observed network across time,
    to be used as an instance variable by the class Analytics.
    """

    def __init__(self):
        """
        Initialize an instance of the Class Historical Network.
        All attributes are set to None.
        """
        self.N = None  # number of banks
        self.T = None  # number of time steps
        self.observed_path = None

    def build_from_data(self, mmsr_data, deposit_rate_data, maintenance_periods_list):
        """
        Build each of the matrices representing the network information from the mmsr panda data frame.
        Store the status of each of these matrices into a dictionary of the observed_path.
        :return: an intance of Class Banks with a adjency matrix of zeros.
        """
        ...
        # we define the number of banks/instituions/others in the network as the number
        # of report agent or counterparties
        self.N = np.size(leis)

        # define the number of periods of the sample from the max and min trade dates in the mmsr database
        target2_calendar = EuropeanCentralBank()
        self.T = (
            target2_calendar.get_working_days_delta(
                mmsr_data.trade_date.min(), mmsr_data.trade_date.max()
            )
            + 1
        )  # add 1 as first period is 0

        # initialise the matrices of the loans, rates, links, as well as the dictionary of their observed path
        y = ma.zeros(
            (self.N, self.N)
        )  # initialisation of the notional matrix with no loans
        r = ma.zeros(
            (self.N, self.N)
        )  # initialisation of the rates matrix with no loans
        r[
            :
        ] = (
            ma.masked
        )  # by default r is masked while y is not, to allow summing the notionals when building
        l = np.zeros(
            (self.N, self.N), int
        )  # initialisation of the links matrix with no links
        network = ClassNetwork(
            y, r, l
        )  # creation of a ClassMatrices object with the matrices of zeros
        self.observed_path = {
            i: deepcopy(network) for i in np.arange(self.T)
        }  # initialisation of a dictionary of the observed paths

        # associate to each date its period number and store it in the new column "period" of the dataframe
        f = lambda x: target2_calendar.get_working_days_delta(
            mmsr_data.trade_date.min(), x.trade_date
        )
        mmsr_data["period"] = mmsr_data.apply(f, axis=1)
        ...
        # create a new column with the spread rate from the ECB deposit facility
        mmsr_data = pd.merge(
            mmsr_data, deposit_rate_data, on="trade_date", how="left"
        )  # first merge data
        mmsr_data["spread"] = (
            mmsr_data.deal_rate - mmsr_data.new_deposit_rate
        )  # compute the spread
        ...
        for mmsr_index in mmsr_data.index:
            ...
            # get the time period from the information on the mmsr data line
            t = mmsr_data.period[mmsr_index]

            # get the counterparties'indexes from the information on the mmsr data line
            if mmsr_data.trns_type[mmsr_index] in ("SELL", "LEND"):
                i = np.where(leis == mmsr_data.report_agent_lei[mmsr_index])[0][0]
                j = np.where(leis == mmsr_data.cntp_lei[mmsr_index])[0][0]

            elif mmsr_data.trns_type[mmsr_index] in ("BORR", "BUYI"):
                j = np.where(leis == mmsr_data.report_agent_lei[mmsr_index])[0][0]
                i = np.where(leis == mmsr_data.cntp_lei[mmsr_index])[0][0]
            ...
            if mmsr_data.end_maintenance_period[mmsr_index] is True:
                self.observed_path[t].end_maintenance = True
            else:
                self.observed_path[t].end_maintenance = False

        # for loop on the period to mask the zeros in the loan size matrices
        for t in np.arange(self.T):
            self.observed_path[t].y = ma.masked_equal(self.observed_path[t].y, 0)

        print(incorect_line_nb, "incorect lines in the data base")
